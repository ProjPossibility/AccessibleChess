...
<<<<<<< HEAD
    ...
    public ChessBoard(Context context, AttributeSet attrs) {
        ...
        darkPaint = new Paint();
        darkPaint.setARGB(255, 0, 0, 255);

        brightPaint = new Paint();
        brightPaint.setARGB(255, 190, 0, 90);
...
=======
	private Position pos;
	private int selectedSquare;
	private int moveSquare;
	private int downSquare;
	private int upSquare;
	private boolean move_b=false;
	private boolean select_b=false;
	...
	final Move mouseMoved(int sq) {
		...
		if(select_b)
		{
			if (moveSquare != sq) 
			{
				Log.i("Helklo","Checking "+sq);
				p = pos.getPiece(sq);
				temp = new Move(selectedSquare, sq, Piece.EMPTY,true);		
				moveSquare=sq;
				return temp;
			}
		}
		else
		{
			if (moveSquare != sq) 
			{
				...
				if (p == Piece.EMPTY) 
				{
					Log.i("heloo","Piece Empty");
				}
				else
				{
					Log.i("heloo","Piece "+p);
					temp = new Move(selectedSquare, sq, Piece.EMPTY);		
				}
				//setSelection(sq);            
				moveSquare=sq;
			}
	...
	final int mouseDown(int sq) {
		...
		if(upSquare == sq&&(SystemClock.elapsedRealtime()-lasttime)<1000)
		{
			if (sq >= 0) 
			{
				p = pos.getPiece(sq);
